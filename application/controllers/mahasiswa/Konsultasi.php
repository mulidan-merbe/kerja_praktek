...
class Konsultasi extends CI_Controller
{
	...
	public function index()
	{
		...
		$data = [
			...
			'proposal' 		=> $this->Model_Proposal->getbyNIM2($NIM),
	...
	public function tambahKonsultasiDosen()
	{
		$NIM = $this->session->userdata('NIM');
		$data = [
			'title' 		=> 'Mahasiswa | Konsultasi Dosen',
		...
		if (empty($_FILES['berkas_file']['name'])) {
			$this->form_validation->set_rules('berkas_file', 'Berkas', 'required');
		}
	...
	{
		$data['title']	= 'Mahasiswa | Konsultasi Dosen';
		$Id_duaA 		= $Id_duaA;
	...
	public function tambahKonsiltasiLapangan()
	{
		$NIM = $this->session->userdata('NIM');
		$data = [
			'title' 		=> 'Mahasiswa | Konsultasi Lapangan',
			'pembimbing' 	=> $this->Model_Pembimbing_lapangan->getDataNIMLimit($NIM)
		];
	...
	{
		$data['title']	= 'Mahasiswa | Konsultasi Lapangan';
		$Id_duaB 		= $Id_duaB;
	...
	public function ubahDataKonsultasiLapangan()
	{
		...
		$data['pembimbing'] = $this->Model_Pembimbing_lapangan->getDataNIMLimit($NIM);
		foreach ($data['pembimbing'] as $data) {
			...
			if (!empty($_FILES['File']['name'])) {
				...
				if (!$this->upload->do_upload('File')) {
					...
					$this->Model_Kpdua_b->ubahData($Id_duaB, $Tema, $Uraian, $No_identitas, $File, $Tanggal);
					$this->session->set_flashdata('flash', 'Diubah');
					redirect('mahasiswa/konsultasi');
				}
				// Kondisi dimana tidak ada file terbaru yg diuploud. Maka yg diuploud adalah file yg lama
				...
				$this->Model_Kpdua_b->ubahData($Id, $Tema, $Uraian, $No_identitas, $File, $Tanggal);
				$this->session->set_flashdata('flash', 'Diubah');
				redirect('mahasiswa/konsultasi');
			}
		}
