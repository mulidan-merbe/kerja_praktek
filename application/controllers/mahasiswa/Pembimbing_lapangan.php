...
class Pembimbing_lapangan extends CI_Controller 
{
	...
	public function tambah()
	{
		...
		// Konfigurasi File
	        ...
	        if (!$this->upload->do_upload('File')) {
				...
				$this->Model_Pembimbing_lapangan->tambahData($Id_proposal, $NIM, $File, $Username, $No_identitas, $Jabatan, $Alamat_kantor, $No_hp, $Tanggal);
				$this->session->set_flashdata('flash', 'Ditambahkan');
				redirect('mahasiswa/pembimbing_lapangan');
	        }
    	}
	}
	...
	public function ubahData()
	{
		...
		if(!empty($_FILES['File']['name']))
		{
			...
			if (!$this->upload->do_upload('File')) {
				...
				$this->Model_Pembimbing_lapangan->ubahData($NIM, $File, $Username, $No_identitas, $Jabatan, $Alamat_kantor, $No_hp, $Tanggal);
	...
	{
		$Id = $_GET['Id'];
		$data = $this->Model_Pembimbing_lapangan->getDatabyId($Id);
		$proses = 'assets/kpifempat/file/'.$data->File;
		if(is_readable($proses) && unlink($proses)) {
			$hapus = $this->Model_Pembimbing_lapangan->hapusData($Id);
